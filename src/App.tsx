  ...
  useEffect(() => {
    ...
    const tryGetSdk = async () => {
      ...
      while (attempts < 20 && !cancelled) {
        const instance = getSdk(MODULE_ID);
        if (instance) {
          try {
...
function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // result is already an AsyncGenerator<string> — no .iterate() needed
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4 scroll-smooth">
              {messages.map((msg) => (
                ...
                >
                  ...
                  <div className="group max-w-[80%]">
                    <div
                      className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed wrap-break-word whitespace-pre-wrap ${
                        msg.role === "user"
                          ? "bg-linear-to-r from-indigo-500 to-purple-600 text-white shadow-md shadow-indigo-500/20"
                          : "bg-gray-800/90 text-gray-100 shadow-sm"
                      }`}
